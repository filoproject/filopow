...
NodeConn: an object which manages p2p connectivity to a filopow node
...
class NodeConnCB:
    """Callback and helper functions for P2P connection to a filopowd node.
...
class NodeConn(asyncore.dispatcher):
    ...
    MAGIC_BYTES = {
        ...
        "regtest": b"\x46\x50\x52\x54",   # FPRT
    }
    ...
    def __init__(self, dstaddr, dstport, rpc, callback, net="regtest", services=NODE_NETWORK, send_version=True):
        ...
        logger.info('Connecting to Filopow Node: %s:%d' % (self.dstaddr, self.dstport))
