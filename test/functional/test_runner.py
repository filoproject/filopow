...
`test/functional/test_framework/test_framework.py:FilopowTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/filopow_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    # Check that the build was configured with wallet, utils, and filopowd
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_filopowd = config["components"].getboolean("ENABLE_FILOPOWD")
    if not (enable_wallet and enable_cli and enable_filopowd):
        print("No functional tests to run. Wallet, utils, and filopowd must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if filopowd is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "filopowd"]) is not None:
            print("%sWARNING!%s There is already a filopowd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "FILOPOWD" not in os.environ:
        os.environ["FILOPOWD"] = build_dir + '/src/filopowd' + exeext
        os.environ["FILOPOWCLI"] = build_dir + '/src/filopow-cli' + exeext
...
class RPCCoverage:
    ...
    commands per `filopow-cli help` (`rpc_interface.txt`).
