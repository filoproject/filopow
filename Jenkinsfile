...
// which are part of the Filopow repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="filopow-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /filopow-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "filopow-filopow/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from filopow-filopow/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'filopow-filopow/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from filopow-filopow/develop"
            }
          }

          if (hasCache) {
            sh "cd /filopow-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /filopow-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /filopow-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /filopow-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /filopow-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /filopow-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /filopow-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
