...
The Filopow Doxygen documentation can be generated by running `make docs` from the top-level 
...
Run with the -testnet option to run with "play filopows" on the test network, if you
...
FILOPOW Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadSocketHandler : Sends/Receives data from peers on port 7767.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 7766 for connections and services them.

- FilopowMiner : Generates filopows (if wallet is enabled).
...
be sent upstream but bugfixes may also be prudent to PR against Filopow so that they can be integrated
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:filopow/filopow.git
