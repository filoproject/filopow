...
*Setup instructions for a Gitian build of FILOPOW Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Filopow
...
to github.com/filoproject/filopow.
...
- [Building FILOPOW Core](#building-filopow-core)
...
Clone the git repositories for FILOPOW Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/The-Filopow-Endeavor/filopow
...
in the FILOPOW Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building FILOPOW Core
----------------

To build FILOPOW Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the FILOPOW Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/filopow/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/The-Filopow-Endeavor/filopow
...
For example:
```bash
URL=https://github.com/crowning-/filopow.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit filopow=${COMMIT} --url filopow=${URL} ../filopow/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit filopow=${COMMIT} --url filopow=${URL} ../filopow/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit filopow=${COMMIT} --url filopow=${URL} ../filopow/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the filopow git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../filopow/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/filopow
SIGPATH=/some/root/path/filopow-detached-sigs

./bin/gbuild --url filopow=${BTCPATH},signature=${SIGPATH} ../filopow/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/filopow-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/filopow-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/filopow-osx-build.assert
```
...
[filopow/gitian.sigs](https://github.com/filopow/gitian.sigs/) repository, or if that's not possible to create a pull
request.
