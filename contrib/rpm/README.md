...
The RPM spec file provided here is for Filopow-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://filopow.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Filopow git repository](https://github.com/filoproject/filopow) but are not part of
...
Currently only `Source10` is used, for the example `filopow.conf` file.
...
in the filopow git repository. At present only an SVG version of the Filopow
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the filopow
...
needed when 0.12.1 is released, a proper fix is already in the Filopow git
master branch.
...
idea to build FILOPOW Core against a static version of that library compiled
...
The `filopow.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `filopow-server` package. At this point, most RPM based
...
The `filopow-server` package should have SELinux support. How to properly do
...
Filopow-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/filopow/). He has been
packaging Filopow for Fedora at least since 2012.
...
4. I renamed the `filopow` package that contains the Qt GUI to `filopow-core` as
that appears to be how the general population refers to it, in contrast to
`filopow-xt` or `filopow-classic`. I wanted to make sure the general population
