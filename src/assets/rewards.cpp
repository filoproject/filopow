...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for FPOW
    ...
    if (p_rewardSnapshot.strDistributionAsset != "FPOW") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is FPOW with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using FPOW differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "FPOW") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Filopow address (already validated during ownership snapshot creation)
