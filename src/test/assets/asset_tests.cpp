...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of FILOPOW NOT allowed
        BOOST_CHECK(!IsAssetNameValid("FPOW", type));
        BOOST_CHECK(!IsAssetNameValid("FILOPOW", type));
        BOOST_CHECK(!IsAssetNameValid("FILOPOW", type));

        //- Versions of FILOPOW ALLOWED
        BOOST_CHECK(IsAssetNameValid("FILOPOW.COIN", type));
        BOOST_CHECK(IsAssetNameValid("FILOPOW_COIN", type));
        BOOST_CHECK(IsAssetNameValid("FPOWSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERFPOW", type));
        BOOST_CHECK(IsAssetNameValid("FILOPOWSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDEFILOPOW", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_FILOPOWS", type));
        BOOST_CHECK(IsAssetNameValid("SEFPOWOT", type));
    ...
    BOOST_AUTO_TEST_CASE(transfer_asset_coin_test)
    {
        ...
        CAssetTransfer asset("FILOPOW", 1000);
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("FILOPOW", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(GetParams().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
