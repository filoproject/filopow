...
bool CScript::IsAssetScript(int& nType, bool& fIsOwner, int& nStartingIndex) const
{
    if (this->size() > 31) {
        if ((*this)[25] == OP_FPOW_ASSET) { // OP_FPOW_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == FPOW_R) { // Check to see if FPOW starts at 27 ( this->size() < 105)
                if ((*this)[28] == FPOW_V)
                    if ((*this)[29] == FPOW_N)
                        index = 30;
            } else {
                if ((*this)[28] == FPOW_R) // Check to see if FPOW starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == FPOW_V)
                        if ((*this)[30] == FPOW_N)
                            index = 31;
            }
