...
#include "filopowgui.h"

#include "filopowunits.h"
...
const std::string FilopowGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString FilopowGUI::DEFAULT_WALLET = "~Default";
...
std::array<CurrencyUnitDetails, 5> CurrencyUnits::CurrencyOptions = { {
    { "BTC",    "FPOWBTC"  , 1,          8},
    { "mBTC",   "FPOWBTC"  , 1000,       5},
    { "µBTC",   "FPOWBTC"  , 1000000,    2},
    { "Satoshi","FPOWBTC"  , 100000000,  0},
    { "USDT",   "FPOWUSDT" , 1,          5}
} };

static bool ThreadSafeMessageBox(FilopowGUI *gui, const std::string& message, const std::string& caption, unsigned int style);

FilopowGUI::FilopowGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
...
void FilopowGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Filopow address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and filopow: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to FPOW addresses"));
...
//    messagingAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Filopow addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Filopow addresses"));
    ...
    openAction->setStatusTip(tr("Open a filopow: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Filopow command-line options").arg(tr(PACKAGE_NAME)));
...
void FilopowGUI::createToolBars()
{
    if(walletFrame)
    {
        QSettings settings;
        ...
        if(IconsOnly) {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/fpowtext")));
        }
        else {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/filopowtext")));
        }
...
//        m_toolbar->addAction(votingAction);
        ...
        QVBoxLayout* filopowLabelLayout = new QVBoxLayout(toolbarWidget);
        filopowLabelLayout->addWidget(labelToolbar);
        filopowLabelLayout->addWidget(m_toolbar);
        filopowLabelLayout->setDirection(QBoxLayout::TopToBottom);
        filopowLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Filopow Market Price"));
        ...
        labelVersionUpdate->setText("<a href=\"https://github.com/filoproject/filopow/releases\">New Wallet Version Available</a>");
        ...
        // Get the latest Filopow release and let the user know if they are using the latest version
        // Network request code for the header widget
                       ...
                       if (key == "tag_name") {
                           ...
                           if (fNewSoftwareFound) {
                               ...
                               if (GetRandInt(2) == 1) {
                                   ...
                                   if (fRet) {
                                       QString link = "https://github.com/filoproject/filopow/releases";
...
void FilopowGUI::updateIconsOnlyToolbar(bool IconsOnly)
{
    if(IconsOnly) {
        labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/fpowtext")));
        m_toolbar->setMaximumWidth(65);
...
void FilopowGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool FilopowGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool FilopowGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void FilopowGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void FilopowGUI::setWalletActionsEnabled(bool enabled)
{
...
void FilopowGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void FilopowGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void FilopowGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void FilopowGUI::optionsClicked()
{
...
void FilopowGUI::showDebugWindow()
{
...
void FilopowGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void FilopowGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void FilopowGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void FilopowGUI::openClicked()
{
...
void FilopowGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void FilopowGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void FilopowGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void FilopowGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void FilopowGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void FilopowGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** FPOW START */
void FilopowGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void FilopowGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void FilopowGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void FilopowGUI::gotoRestrictedAssetsPage()
{
...
void FilopowGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Filopow network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void FilopowGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void FilopowGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void FilopowGUI::updateHeadersSyncProgressLabel()
{
...
void FilopowGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void FilopowGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Filopow"); // default title
...
void FilopowGUI::changeEvent(QEvent *e)
{
...
void FilopowGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void FilopowGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void FilopowGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "FPOW")
        msg += tr("Amount: %1\n").arg(FilopowUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(FilopowUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void FilopowGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void FilopowGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool FilopowGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool FilopowGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void FilopowGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void FilopowGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void FilopowGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void FilopowGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void FilopowGUI::detectShutdown()
{
...
void FilopowGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void FilopowGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void FilopowGUI::showModalOverlay()
{
...
static bool ThreadSafeMnemonic(FilopowGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void FilopowGUI::subscribeToCoreSignals()
{
...
void FilopowGUI::unsubscribeFromCoreSignals()
{
...
void FilopowGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void FilopowGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<FilopowUnits::Unit> units = FilopowUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const FilopowUnits::Unit unit : units)
    {
    #ifndef QTversionPreFiveEleven
        max_width = qMax(max_width, fm.horizontalAdvance(FilopowUnits::name(unit)));
    #else
        max_width = qMax(max_width, fm.width(FilopowUnits::name(unit)));
    #endif
    }
...
{
    menu = new QMenu(this);
    for (FilopowUnits::Unit u : FilopowUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(FilopowUnits::name(u)), this);
...
{
    setText(FilopowUnits::name(newUnits));
}
...
void FilopowGUI::currencySelectionChanged(int unitIndex)
{
    if(clientModel && clientModel->getOptionsModel())
    {
...
void FilopowGUI::onCurrencyChange(int newIndex)
{
    qDebug() << "FilopowGUI::onPriceUnitChange: " + QString::number(newIndex);
...
void FilopowGUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/FilopowProject/Filopow/releases"));
