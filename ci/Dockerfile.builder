...
# filopow_hash
RUN git clone https://github.com/The-Filopow-Endeavor/filopow_hash
RUN cd filopow_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} filopow
RUN useradd -u ${USER_ID} -g filopow -s /bin/bash -m -d /filopow filopow
...
RUN mkdir /filopow-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /filopow-src && \
