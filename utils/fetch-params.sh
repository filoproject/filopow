...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/FilopowParams"
else
    PARAMS_DIR="$HOME/.filopow-params"
fi
...
DOWNLOAD_URL="https://github.com/The-Filopow-Endeavor/filopow/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with FPOW_DISABLE_SOMETHING=1
FPOW_DISABLE_WGET="${FPOW_DISABLE_WGET:-}"
#FPOW_DISABLE_IPFS="${FPOW_DISABLE_IPFS:-}"
FPOW_DISABLE_CURL="${FPOW_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$FPOW_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$FPOW_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$FPOW_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Filopow parameters!
...
main() {
    ...
    cat <<EOF
FPOW - fetch-params.sh

This script will fetch the Filopow SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Filopow SNARK parameters. Note that it is
